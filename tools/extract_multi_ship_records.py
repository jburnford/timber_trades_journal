#!/usr/bin/env python3
"""
Phase 2: Extract ships from multi-ship records (HIGH, MEDIUM, LOW: 300-10K chars).
Uses same @ symbol splitting approach as MEGA extraction.
"""
import csv
import sys
import re
...
class ShipRecord:
    ...
    is_steamship: bool
    raw_segment: str
    extraction_method: str  # 'MULTI_SPLIT'
    needs_review: bool      # True for multi-ship extractions
    original_extracted_ship: str  # The ship that was originally extracted

def extract_ships_from_multi_line(raw_line: str, source_file: str, line_number: int,
                                   destination_port: str, pub_date: Tuple,
                                   original_ship: str) -> List[ShipRecord]:
    """
    Extract multiple ships from a concatenated line using @ symbol boundaries.
    """
    ships = []
    ...
    for idx, at_pos in enumerate(at_positions):
        try:
            ...
            ships.append(ShipRecord(
                ...
                cargo=cargo,
                merchant=merchant,
                arrival_day=None,  # Dates not reliably extractable from concatenated records
                arrival_month=None,
...
def extract_multi_ship_records(categories_csv: Path, main_csv: Path, output_csv: Path):
    """Extract ships from HIGH, MEDIUM, and LOW category records."""

    print("="*80)
    print("PHASE 2: EXTRACTING SHIPS FROM MULTI-SHIP RECORDS")
    print("="*80)

    # Step 1: Read categorization to find multi-ship records (HIGH, MEDIUM, LOW)
    print("\nStep 1: Finding multi-ship records (HIGH, MEDIUM, LOW categories)...")
    multi_records = []

    with open(categories_csv, 'r', encoding='utf-8') as f:
        reader = csv.DictReader(f)
        for row in reader:
            if row['size_category'] in ['HIGH', 'MEDIUM', 'LOW']:
                # Only process records with @ symbols (standard format)
                if int(row['at_symbols']) > 0:
    ...
    print(f"Found {len(multi_records)} multi-ship records with standard format")

    # Count by category
    from collections import Counter
    category_counts = Counter(r['size_category'] for r in multi_records)
    for cat, count in sorted(category_counts.items()):
        estimated = sum(r['estimated_ships'] for r in multi_records if r['size_category'] == cat)
        print(f"  {cat:8} : {count:4} records | {estimated:5,} estimated ships")
    ...
    with open(main_csv, 'r', encoding='utf-8') as f:
        reader = csv.DictReader(f)
        for row in reader:
            # Check if this row matches any multi record
            for multi in multi_records:
                if (row['source_file'] == multi['source_file'] and
                    int(row['line_number']) == multi['line_number']):
                    multi_data[multi['line_number']] = {
    ...
    print(f"Loaded {len(multi_data)} multi-ship raw_line entries")

    # Step 3: Extract ships from each multi-ship record
    print("\nStep 3: Extracting ships from multi-ship records...")
    all_extracted_ships = []

    for idx, multi in enumerate(multi_records, 1):
        line_num = multi['line_number']
        if line_num not in multi_data:
            print(f"  WARNING: Could not find raw_line for {multi['source_file']}")
            continue

        data = multi_data[line_num]
        raw_line = data['raw_line']

        if idx % 100 == 0 or multi['size_category'] == 'HIGH':
            print(f"  [{idx:4}/{len(multi_records)}] {multi['source_file'][:50]:50} | {multi['size_category']:6} | Expected: {multi['estimated_ships']:3}")

        ships = extract_ships_from_multi_line(
            raw_line=raw_line,
            source_file=multi['source_file'],
    ...
    # Summary
    print("\n" + "="*80)
    print("MULTI-SHIP RECORD EXTRACTION SUMMARY")
    print("="*80)
    print(f"Multi-ship records processed: {len(multi_records)}")
    print(f"Total ships extracted:        {len(all_extracted_ships):,}")
    print(f"Expected ships:               {sum(m['estimated_ships'] for m in multi_records):,}")

    expected_total = sum(m['estimated_ships'] for m in multi_records)
    if expected_total > 0:
...
if __name__ == '__main__':
    ...
    output_csv = Path("/home/jic823/TTJ Forest of Numbers/parsed_output/multi_ship_extracted.csv")

    extract_multi_ship_records(categories_csv, main_csv, output_csv)
