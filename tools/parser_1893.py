#!/usr/bin/env python3
"""
Year-specific parser for 1893 timeout groups.
...
def parse_line_1893(line: str, line_num: int, current_port: str,
                    last_date: Tuple, pub_date: Tuple, source_file: str) -> List[ShipRecord]:
    """
    Parse 1893 format with SIMPLE string operations.
...
def parse_file_1893(file_path: Path) -> List[ShipRecord]:
    """Parse single 1893 file with simple patterns."""
    records = []
    ...
    if match:
        ...
        pub_year, pub_month, pub_day = 1893, 1, 1
...
def process_1893_timeout_groups(timeout_list: Path, ocr_dir: Path, output_file: Path):
    """Process all 1893 timeout groups."""
    ...
    print(f"Processing {len(base_names)} 1893 timeout groups...")
    ...
    for i, base_name in enumerate(base_names, 1):
        ...
        for page_file in page_files:
            try:
                records = parse_file_1893(page_file)
...
if __name__ == '__main__':
    timeout_list = Path("/home/jic823/TTJ Forest of Numbers/parsed_output/timeout_clusters/timeout_1893.txt")
    ocr_dir = Path("/home/jic823/TTJ Forest of Numbers/ocr_results/gemini_full")
    output_file = Path("/home/jic823/TTJ Forest of Numbers/parsed_output/timeout_1893_records.csv")

    process_1893_timeout_groups(timeout_list, ocr_dir, output_file)
