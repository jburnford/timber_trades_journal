#!/usr/bin/env python3
"""
Year-specific parser for REMAINING timeout groups.
...
def parse_line_REMAINING(line: str, line_num: int, current_port: str,
                    last_date: Tuple, pub_date: Tuple, source_file: str) -> List[ShipRecord]:
    """
    Parse REMAINING format with SIMPLE string operations.
...
def parse_file_REMAINING(file_path: Path) -> List[ShipRecord]:
    """Parse single REMAINING file with simple patterns."""
    records = []
    ...
    if match:
        ...
        pub_year, pub_month, pub_day = REMAINING, 1, 1
    ...
    try:
        with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
            for line_num, line in enumerate(f, 1):
                ...
                if current_port:
                    ship_records = parse_line_REMAINING(
...
def process_REMAINING_timeout_groups(timeout_list: Path, ocr_dir: Path, output_file: Path):
    """Process all REMAINING timeout groups."""
    ...
    print(f"Processing {len(base_names)} REMAINING timeout groups...")
    ...
    for i, base_name in enumerate(base_names, 1):
        ...
        for page_file in page_files:
            try:
                records = parse_file_REMAINING(page_file)
    ...
    print(f"\n{'='*80}")
    print(f"REMAINING PARSER RESULTS")
    print(f"{'='*80}")
...
if __name__ == '__main__':
    timeout_list = Path("/home/jic823/TTJ Forest of Numbers/parsed_output/timeout_clusters/timeout_remaining_years.txt")
    ocr_dir = Path("/home/jic823/TTJ Forest of Numbers/ocr_results/gemini_full")
    output_file = Path("/home/jic823/TTJ Forest of Numbers/parsed_output/timeout_remaining_records.csv")

    process_REMAINING_timeout_groups(timeout_list, ocr_dir, output_file)
